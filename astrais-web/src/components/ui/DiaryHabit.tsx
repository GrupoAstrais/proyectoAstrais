...
export default function ButtonFilter({handleActive, titulo, esOtroActivo, active: activeProp} : ButtonProps) {
    ...
    return (
        <>
            <button type="button" onClick={changeState} className={`rounded-xs px-2 shadow-xs shadow-primary-900 ${currentActive ?  'translate-y-1 bg-secondary-700 text-white shadow-none' : 'bg-state-success text-primary-900' }`}>
