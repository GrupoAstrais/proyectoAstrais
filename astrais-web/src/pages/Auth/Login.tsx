...
export default function Login() {
  ...
  useEffect(() => {
    ...
    const jwtAccessToken =
      params.get("jwtAccessToken") ?? params.get("accessToken");
    const jwtRefreshToken =
      params.get("jwtRefreshToken") ?? params.get("refreshToken");

    console.log("Params encontrados:", {
      uid,
      hadToRegister,
  ...
  const onSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await performLogin({ email: email, passwd: password })
      .then(() => {
        navigate("/home");
      })
      .catch(() => {
    ...
    <section className="relative grid min-h-screen place-items-center overflow-hidden px-6 max-[480px]:px-4">
      ...
      >
        <header className="mb-5.5">
          <p className="m-0 text-xs tracking-[0.2em] text-indigo-200 font-bold uppercase">
            Astrais
          </p>
          <h1
            id="login-title"
            className="my-2 text-[clamp(1.9rem,4vw,2.35rem)] leading-[1.1]"
          >
            Log in
          </h1>
          <p className="m-0 text-[color-mix(in_srgb,var(--astrais-text)_90%,transparent)]">
        ...
        <form className="grid gap-4" onSubmit={onSubmit} noValidate>
          ...
          {error ? (
            <p
              className="m-0 rounded-[10px] border border-[color-mix(in_srgb,var(--astrais-error)_55%,transparent)] bg-[color-mix(in_srgb,var(--astrais-error)_20%,transparent)] p-2.5 text-[0.9rem]"
              role="status"
              aria-live="polite"
            >
              {error}
